#!/usr/bin/env python3
"""
PyTorch implementation of BP-based multitarget tracking.
Converted from MATLAB implementation by Florian Meyer, 2017, 2020.
...
def main():
    ...
    parameters['target_start_states'] = data_gen.get_start_states(5, 1000, 10).to(device)
    ...
    parameters['sensor_positions'] = data_gen.get_sensor_positions(parameters['num_sensors'], 5000).to(device)
    
    # Move other parameters to device
    for key in ['surveillance_region', 'prior_velocity_covariance', 'length_steps']:
        parameters[key] = parameters[key].to(device)
    ...
    unknown_number = 0.01
    unknown_particles = torch.zeros(2, parameters['num_particles'], device=device)
    surveillance_region = parameters['surveillance_region']
    unknown_particles[0, :] = (torch.rand(parameters['num_particles'], device=device) * 
                              (surveillance_region[1, 0] - surveillance_region[0, 0]) + 
                              surveillance_region[0, 0])
    unknown_particles[1, :] = (torch.rand(parameters['num_particles'], device=device) * 
    ...
    for est in estimates:
        if 'state' in est and len(est['state']) > 0:
            if isinstance(est['state'], torch.Tensor):
    ...
    try:
        ...
        for target in range(min(5, true_tracks.shape[1])):
            track = true_tracks[:2, target, :].cpu().numpy()
        ...
        plt.subplot(2, 2, 4)
        sensor_pos = parameters['sensor_positions'].cpu().numpy()
        plt.scatter(sensor_pos[0, :], sensor_pos[1, :], s=100, c='red', marker='s', label='Sensors')
        
        # Plot surveillance region
        surv_region = parameters['surveillance_region'].cpu().numpy()
        ...
        plt.tight_layout()
        plt.savefig('/Users/lipingb/Desktop/BP-MTT/tracking_results.png', dpi=150, bbox_inches='tight')
        plt.show()
